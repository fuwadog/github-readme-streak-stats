...
**ROUTE_CONTRACT:** The canonical production URL is `https://github-readme-streak-stats-black-phi.vercel.app`; `https://streak-stats.demolab.com` remains a legacy alias for existing embeds. `/` serves the card API, `/demo/` serves the demo, and `/demo/<asset>` serves demo assets. These public routes and their query behavior must remain unchanged. Direct PHP files are not public routes, and protected previews or deployment URLs must not be used as embed hosts.

### What does `WHITELIST` control?

**WHITELIST_CONTRACT:** `WHITELIST` is an explicit comma-separated list of GitHub usernames allowed by a deployment. Configure the exact usernames intended to be served; a non-listed username receives `403`. A missing, empty, wildcard, or malformed value fails closed, so every username receives `403` until a valid list is configured. This API policy is unrelated to GitHub collaborator permissions and does not grant anyone repository access.

### What are the cache, limiter, and token policies?

Successful cards are cacheable for the configured `CACHE_TTL` (which takes precedence over `CACHE_TTL_DEFAULT`); `DISABLE_CACHE=true` and all error responses use `no-store`. The checked-in `.env.example` sets a five-hour local default, while the runtime falls back to one day when neither TTL is set. `TOKEN` and `TOKEN2` through `TOKEN100` are server-side failover credentials only and must be encrypted Production variables on Vercel; rotate by adding the replacement, redeploying and verifying, then revoking and removing the old value. Never put token values in URLs, arguments, manifests, or logs.

**LIMIT_CONTRACT:** Self-hosted deployments use a file-based limit of 100 requests per minute per client IP. Vercel's isolated filesystem cannot provide a shared limiter, so production serverless traffic requires Vercel or an upstream WAF/API gateway. Set `EXTERNAL_RATE_LIMITER=true` only after that limiter is active. For the 2026 Vercel Hobby WAF configuration, each project has one project rule; document and verify it rather than assuming a platform-wide default. Its supported window is fixed between 10 seconds and 10 minutes, counters may use client IP or JA4 keys, and counters are regional rather than globally shared. Choose the key, window, and regions deliberately for the deployment; there is no assumed global default. Before release, manually retain the project/deployment identity and dashboard evidence for the rule, including its key, window, and regions; repository fail-closed tests do not establish deployed WAF coverage. The current canonical production whitelist is user-managed and includes `fuwadog`; a separate deployment should use its own exact allowlist.
